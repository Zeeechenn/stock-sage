...
- M30.5 dependency audit 收口（2026-06-02）：`efinance` 已从默认依赖移到 optional extra，CN 日线与指数 provider 仅在安装后注册；默认环境不再带入 `retry -> py`，`pytest` 升至 9.0.3，`uv.lock` 已重锁，`make python-lock-check` 与 `make dependency-audit` 通过且无已知漏洞。前端已小步升级到 Vite 6.4.3 / `@vitejs/plugin-react` 4.7，`npm audit` 为 0 vulnerabilities，未使用 force 跳 Vite 8。剩余 M30 债务为非 CLI logger 化、AdminPage 与 test2 runner 后续拆分。
