...
**目录**：[当前发布](#-当前发布) · [核心特性](#-核心特性) · [快速开始](#-快速开始) · [推荐使用方式](#-推荐使用方式) · [Agent 使用指南](#-agent-使用指南) · [配置](#-配置) · [架构预览](#-架构预览) · [未来开发计划](#-未来开发计划) · [注意事项](#-注意事项) · [更多文档](#-更多文档) · [风险声明](#-风险声明)

---

## 📦 当前发布

**v0.2.0（2026-05-31）** 是一次 agent-ready / research-runtime release：原生 `stocksage` Pi 终端、CLI / MCP 上下文入口、M28 dossier / deep research / copilot 信息流、Tushare qfq late fallback 与 iFinD MCP observe-only adapter 已进入公开主线。

M26 / M27 的量化与 Kronos 证据闭环也随本版公开：所有候选均未通过 production promotion gate，生产默认继续 `WEIGHT_QUANT=0.0` / `kronos_enabled=false`。下一阶段进入 M29 Forward Evidence Engine，重点是只读证据账本与预注册 alpha 假设，而不是把未达标候选包装进生产。
...
| 🔗 **多源行情** | Eastmoney / AkShare 默认 fallback；可选接入 efinance、Tushare qfq、yfinance、TickFlow、Tavily、Anspire、iFinD MCP observe-only。 |
| 🧩 **分层信号** | 技术指标 + LLM 新闻情绪 + 长期分析师团 + 组合风控，每层独立、可单独复盘。 |
| 📒 **可审计记忆** | 项目记忆、分层决策记忆、聊天摘要分桶落库，每次写入都有审计日志。 |
| 🛡 **风险优先** | 止盈止损全部由 ATR 公式与组合约束生成，LLM 不直接预测价格。 |

> 当前聚焦 **M29 forward evidence 与预注册 alpha 假设**、agent 化使用体验、数据质量与研究复盘能力；不是自动交易系统，Web 控制台仍在演进中。
