...
明仓把"看好一只票"做成一条会回看、会认错、会沉淀的闭环——信号只是起点，**最值钱的，是它从结果里学到的东西**。出结果后它会归因、记分，把教训沉淀成下次能用的记忆。

> ### 📌 真实样本：一笔亏损如何变成一条新规则
> **2026-05 · 宁德时代 · 纸上交易**
>
> 1. **信号入场** 05-14 @ 449.38，止损 395.57。
> 2. **信号一路转弱**，但当时没有"信号反转就退出"的规则，只能硬扛 →
> 3. **主动平仓** 05-25 @ 411.28，认亏 **−8.48%**。
> 4. **复盘归因** 写下根因：*缺少信号反转退出规则*（不是机械止损的问题）。
> 5. **沉淀成改进** 这条教训**直接催生了测试2的"信号反转退出"新规则**。
>
> 👉 亏的钱没白亏，它变成了系统记忆里的一条规则。
> [看完整链路 → 宁德活样本](docs_public/ningde_live_sample.md)
...
| 002050 | 三花智控 | −7.9 | ⚪ 观望 | 6.4 | 5.2 | −85.3 | — | — |

> 只给分档和纪律位，**不喊"买入/必涨"**。新闻情感由 LLM 当日读盘打分；止盈止损是 ATR 公式算的规则。当前生产信号是一套朴素公式（技术 0.6 + 新闻情感 0.4 + ATR 2.5 移动止损）——**真正的价值在上面那条会学习的闭环里。**
...
│   🛑 止损 5 笔   平均 −5.33%（最大 −9.20%，无一笔失控） │
│                                                        │
│   抓大放小：赚 +22.8% / 亏 −5.3% ≈ 4.3 : 1            │
│                                                        │
└─ 纸上交易回放·非真金白银 · 历史结果不回改 ─────────────┘
```

> **我们把每一笔止损都如实列出来。** 这 5 笔止损全部砍在小亏（最大 −9.2%），2 笔盈利放到 +22.8%——亏损不是污点，是 ATR 止损纪律在生效的证据。
>
> 🔎 这一切怎么做到的？往下看 [研究决策闭环架构](#架构研究决策闭环) —— L0 记忆 · L1 证据 · L2 论题 · L3 信号/持仓 · L4 复盘校准。
